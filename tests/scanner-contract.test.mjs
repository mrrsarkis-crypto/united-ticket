...
test('Cloudflare Workers AI OCR uses one Gemma document pass before external fallbacks', async () => {
  ...
  const env = {
    AI: {
      run: async (name, input, runOptions) => {
        calls++;
        model = name;
        payload = input;
        options = runOptions;
        return { choices: [{ message: { content: '{"citationNumber":"J533157","confidentFields":["citationNumber"],"unknownFields":[],"legibility":"fair"}' } }] };
      },
    },
    GROQ_API_KEY: 'test-groq',
    SCANNER_PROVIDER_TIMEOUT_MS: '8000',
...
test('Workers AI Gemma pass preserves essential ticket fields in one structured read', async () => {
  let calls = 0;
  const env = {
    AI: {
      run: async () => {
        ...
        return { choices: [{ message: { content: JSON.stringify({
          citationNumber: 'J533157',
          violationCode: '22350',
          violationDescription: 'speeding',
          violationDate: '10/11/24',
          dueDate: '01/20/26',
          defendantName: 'SAMVEL REZYAN',
          drivingLicenseNumber: null,
          drivingLicenseState: 'CA',
          courtDate: null,
          courtOrAgency: 'LOS ANGELES POLICE DEPARTMENT / CITY OF LOS ANGELES',
          location: null,
          vehiclePlate: null,
          confidentFields: ['citationNumber','violationCode','dueDate'],
          unknownFields: ['drivingLicenseNumber','courtDate','location','vehiclePlate'],
          legibility: 'fair'
        }) } }] };
      },
    },
  ...
  assert.equal(extracted.citationNumber.confident, true);
  assert.equal(extracted.violationCode.confident, true);
  assert.equal(extracted.dueDate.confident, true);
});
